...
// Convenience macro which creates a "using" statement, a test suite, and automatically registers the created test suite at the end of the current scope.
...
// Registers a file to output results to. No file is generated by default.
void set_ofstream(std::ofstream *);

// Sets the indentation sequence for printed assertions.
void set_indentation(char const *);

// If true, passing assertions will be printed - they are not printed by default. Failing assertions are always printed.
...
// Registers a suite for later evaluation by `test::evaluate_suites`. Takes an r-value reference - the passed suite will be cannibalized! If `TEST_THREADSAFE_REGISTRATION_AND_EVALUATION` is non-zero, this operation is threadsafe.
void register_suite(Suite &&);

// Evaluates all currently registered suites prints their results. After evaluation, all suites are cleared. If `TEST_THREADSAFE_REGISTRATION_AND_EVALUATION` is non-zero, this operation is threadsafe.
void evaluate_suites();

// Returns the total number of assertions passed thus far.
size_t assertions_passed();

// Returns the total number of assertions failed thus far.
